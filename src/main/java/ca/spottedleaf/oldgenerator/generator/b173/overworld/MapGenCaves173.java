...
public class MapGenCaves173 extends MapGenBase173 {
    ...
    protected void generate(int i, int j, ChunkGenerator.ChunkData chunkData, double d0, double d1, double d2) {
        this.generate(i, j, chunkData, d0, d1, d2, 1.0F + this.random.nextFloat() * 6.0F, 0.0F, 0.0F, -1, -1, 0.5D);
    }

    protected void generate(int i, int j, ChunkGenerator.ChunkData chunkData, double d0, double d1, double d2, float f, float f1, float f2, int k, int l, double d3) {
                    ...
                    for (j3 = k1; !flag2 && j3 < l1; ++j3) {
                        for (int k3 = k2; !flag2 && k3 < l2; ++k3) {
                            for (int l3 = j2 + 1; !flag2 && l3 >= i2 - 1; --l3) {
                                i3 = (j3 * 16 + k3) * 128 + l3;
                                if (l3 >= 0 && l3 < 128) {
                                    if (LegacyUtil173.getType(chunkData, i3) == Material.WATER) {
                                        flag2 = true;
                                    }
                                ...
                                if (d12 * d12 + d13 * d13 < 1.0D) {
                                    for (int j4 = j2 - 1; j4 >= i2; --j4) {
                                        ...
                                        if (d14 > -0.7D && d12 * d12 + d14 * d14 + d13 * d13 < 1.0D) {
                                            ...
                                            if (b0 == Material.GRASS_BLOCK) {
                                                flag3 = true;
                                            }

                                            if (b0 == Material.STONE || b0 == Material.DIRT || b0 == Material.GRASS_BLOCK) {
                                                if (j4 < 10) {
                                                    LegacyUtil173.setBlockData(chunkData, i4, BlockConstants.SOURCE_LAVA);
                                                } else {
                                                    LegacyUtil173.setBlockData(chunkData, i4, BlockConstants.AIR);
                                                    if (flag3 && LegacyUtil173.getType(chunkData, i4 - 1) == Material.DIRT) {
                                                        LegacyUtil173.setBlockData(chunkData, i4 - 1, BlockConstants.GRASS_BLOCK);
                                                    }
                                                }
                                            }
                                        }
